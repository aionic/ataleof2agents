...
# Get Weather API URL
$WEATHER_API_URL = az containerapp show `
    --resource-group $env:AZURE_RESOURCE_GROUP `
    --query defaultHostName -o tsv

# Save to .env
Add-Content ../../.env "`nWEATHER_API_URL=$WEATHER_API_URL"
```
...
|----------|---------|--------------|
| `WEATHER_API_URL` | Weather API endpoint | Weather API deployment |
...
3. **Query existing resources for URLs:**
   ...
   # Add to .env
    Add-Content .env "`nWEATHER_API_URL=https://$WEATHER_API_URL"
   ```
...
Add-Content .env "`nWEATHER_API_URL=$FUNCTION_URL"
```
